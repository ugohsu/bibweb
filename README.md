...
- **詳細検索**: 検索ボックス下の「詳細検索」アコーディオン（既定は折りたたみ）で title/author/year/journal を個別指定して絞り込めます。フィールドをまたぐ組み合わせは常に AND、フィールド内は `|` で OR・半角スペースで AND（year のみ `|` による OR・範囲指定のみで AND 非対応）。ここで絞った母集団に対してさらに曖昧検索もかけられます（詳細は後述）
...
- 詳細検索: 検索ボックスの下の「詳細検索」を開くと、title/author/year/journal を個別指定して絞り込めます。組み合わせは常に AND、各フィールド内は `|` で OR・半角スペースで AND（記法の例はフォーム内のヒント表示を参照。year のみ `|` による OR・範囲指定のみで AND 非対応）。この絞り込みに対してさらに曖昧検索を重ねられます
- 各エントリにタグがあればピルとして表示されます。ピルをクリックするとそのタグで絞り込みます
- チェックボックスで複数選択 → `.bib` または `.db` として書き出し・一括タグ追加。「選択のみ」チェックボックスでチェック済みだけに絞り込めます（タグ・検索・詳細検索を切り替えながら選んだエントリを見失わないため。チェックが0件になると自動でOFFに戻ります）。✕ボタンで選択解除できます
