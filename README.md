...
- **`.db` エクスポート**: 選択したエントリを `extras` ごと bibdb 互換の SQLite ファイルとして書き出せます
...
**Extras タブ**: `extras` テーブルの全行を表示・編集・削除・追加できる raw ビューです。`tags`・`file`・`md.*` キーも含めてすべて表示されます。
...
| `.db` のエクスポート | — | ✅（選択エントリを GUI から・extras ごと） |
| フィールドの編集 | — | ✅ |
