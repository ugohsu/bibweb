...
- **カスタムビュー**: 各プロジェクト側で作った文献横断的な静的HTML（マインドマップ・ダッシュボード等）を、ヘッダーの「カスタムビューを開く」から読み込んで表示できます。そのHTMLから `/?entry=<cite_key>` 形式のリンクで特定エントリへ直接飛べます（詳細は後述）
...
`/?entry=<cite_key>` の形式でアクセスすると、起動時にそのエントリを自動選択して詳細パネルを開きます。特定の論文へのリンクを人に共有したり、後述のカスタムビューから該当エントリへ飛ばしたりするための入口です。

```
http://localhost:8766/?entry=Smith_2024_disclosure
```

対象の `cite_key` が DB に存在しない場合は何も起きず、通常のエントリ一覧が表示されます。
...
- 読み込んだHTMLは `<iframe srcdoc="...">` に流し込んで表示します（サーバーへのアップロードは発生しません）。`sandbox="allow-scripts allow-top-navigation allow-popups allow-forms allow-modals"` を指定しており、カスタムビュー側のスクリプトは実行できますが、bibweb本体のDOM・状態には直接アクセスできません（`allow-same-origin` を付けていないため）。
- カスタムビュー側から特定の論文へリンクするには、上記のディープリンクを使い、`target="_top"` を指定します。
  ```html
  <a href="/?entry=Smith_2024_disclosure" target="_top">Smith (2024)</a>
  ```
  `srcdoc` で埋め込まれた文書内の相対URL（`/?entry=...` のようなルート相対パス）は、仕様上「埋め込み元ページのURL」を基準に解決されるため、**bibweb がどのポート（`--port`）で動いていてもポート番号を書き込む必要はありません**。
- カスタムビューのHTML自体は完全に自己完結（データ・CSS・JSをすべてインライン化）である必要があります。相対パスで別ファイル（`./chart.js` 等）を参照する構成は `srcdoc` では読み込めません。
