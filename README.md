...
**Extras タブ**: `extras` テーブルのうち bibweb が特別扱いするキー（`tags` / `md.*`）を除いた行の表示・編集・削除・追加。
...
| `extra_key` | 扱い |
|---|---|
| **`tags`** | **Tags タブで管理**。1 エントリに複数行持てます（1 行 = 1 タグ）。Extras タブには表示されません。 |
| **`md.digest`** | **Fields タブに分割表示**。Fields タブを開いたとき右側にダイジェストを並べて表示します。Markdown タブでも表示されます。 |
| `md.*`（`md.` で始まる全キー） | **Markdown タブで表示**。Extras タブには表示されません。 |

これら以外のキー（例: `memo`, `file`, `url`）は Extras タブで自由に管理できます。
